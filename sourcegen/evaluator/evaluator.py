...
class Evaluator:
    def __init__(self, dset, is_model_manifold):
        self.dset = dset
    ...
    def score_clf_model(self, scores_raw, obj_truth, bias=0.0):
        # convert scores logits to probabilities
        if scores_raw[0].sum().int().item() != scores_raw[0].shape[0]:
            scores_raw = [torch.nn.functional.softmax(scores_raw[0], dim=1),
                          torch.nn.functional.softmax(scores_raw[1], dim=1)]

        attr_pred, obj_pred = scores_raw
